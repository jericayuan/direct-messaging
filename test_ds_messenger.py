...
def test_retrieve_new(messenger, mock_socket):
    ...
    mock_response = {
        "response": {
            "type": "ok",
            "messages": [
                {"from": "user1", "entry": "Hi!", "timestamp": "2025-02-28T12:34:56"},
                {"from": "user2", "entry": "Hello!", "timestamp": "2025-02-28T12:35:00"}
            ]
        }
    }
    mock_client.recv.return_value = json.dumps(mock_response).encode()
...
def test_client_socket(messenger, mock_socket):
    mock_conn = MagicMock()
    mock_socket.return_value  = mock_conn
    join_response = {
        "response": {"type": "ok", "message": "Welcome back!", "token": ""}
    }
